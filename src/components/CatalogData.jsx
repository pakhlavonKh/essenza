 const catalog = {
        "cellImage_0_0": {
            "brand": "Chanel",
            "name": "Chanel eau Tendre",
            "description": "Deluxe"
        },
        "cellImage_0_1": {
            "brand": "Chanel",
            "name": "Allure Homme Sport",
            "description": "Standard"
        },
        "cellImage_0_2": {
            "brand": "Chanel",
            "name": "Bleu de Chanel",
            "description": "Deluxe"
        },
        "cellImage_0_3": {
            "brand": "Chanel",
            "name": "Egoiste Platinum",
            "description": "Standard"
        },
        "cellImage_0_4": {
            "brand": "Tom Ford",
            "name": "Cherry Smoke",
            "description": "Deluxe"
        },
        "cellImage_0_6": {
            "brand": "Tom Ford",
            "name": "Black Orchid",
            "description": "Top"
        },
        "cellImage_0_7": {
            "brand": "Tom Ford",
            "name": "Lost Cherry",
            "description": "Standard, Deluxe"
        },
        "cellImage_0_8": {
            "brand": "Tom Ford",
            "name": "Tobacco Vanille",
            "description": "Top, Deluxe"
        },
        "cellImage_0_9": {
            "brand": "Tom Ford",
            "name": "Ombre Leather ",
            "description": "Standard, Deluxe"
        },
        "cellImage_0_10": {
            "brand": "Tom Ford",
            "name": "Vanilla Sex",
            "description": "Top"
        },
        "cellImage_0_11": {
            "brand": "Louis Vuitton",
            "name": "L'Immensite",
            "description": "Top, Deluxe"
        },
        "cellImage_0_12": {
            "brand": "Louis Vuitton",
            "name": "Afternoon Swim",
            "description": "top, Deluxe"
        },
        "cellImage_0_13": {
            "brand": "Louis Vuitton",
            "name": "Ombre Nomade",
            "description": "Standard,Top, Deluxe"
        },
        "cellImage_0_14": {
            "brand": "Louis Vuitton",
            "name": "Rose des Vents",
            "description": "Top"
        },
        "cellImage_0_15": {
            "brand": "Dior",
            "name": "Miss Dior Blooming Bouquet",
            "description": "Top, Deluxe"
        },
        "cellImage_0_16": {
            "brand": "Dior",
            "name": "Sauvage Elixir",
            "description": "Deluxe"
        },
        "cellImage_0_17": {
            "brand": "Lacoste",
            "name": "Lacoste Sport",
            "description": "Standard"
        },
        "cellImage_0_18": {
            "brand": "Lacoste",
            "name": "Lacoste Femme",
            "description": "Deluxe"
        },
        "cellImage_0_19": {
            "brand": "Lacoste",
            "name": "Lacoste White",
            "description": "Standard, Top"
        },
        "cellImage_0_20": {
            "brand": "Ex Nihilo",
            "name": "Blue Talisman",
            "description": "Standard, Deluxe"
        },
        "cellImage_0_21": {
            "brand": "Ex Nihilo",
            "name": "Fleur Narcotique",
            "description": "Standard, Top"
        },
        "cellImage_0_22": {
            "brand": "Creed",
            "name": "Silver Mountain Water",
            "description": "Standard"
        },
        "cellImage_0_23": {
            "brand": "Creed",
            "name": "Absolu Aventus",
            "description": "Top, Deluxe"
        },
        "cellImage_0_24": {
            "brand": "Tiziana Terenzi",
            "name": "Kirke",
            "description": "Standard, Deluxe"
        },
        "cellImage_0_25": {
            "brand": "Nasomatto",
            "name": "Black Afgano",
            "description": "Top, Deluxe"
        },
        "cellImage_0_26": {
            "brand": "Dolce&Gabbana",
            "name": "L'Impératrice",
            "description": "Standard, Top"
        },
        "cellImage_0_27": {
            "brand": "Maison Francis Kurkdjan",
            "name": "Baccarat Rouge 540",
            "description": "Top"
        },
        "cellImage_0_28": {
            "brand": "Maison Francis Kurkdjan",
            "name": "Oud Satin Mood",
            "description": "Top"
        },
        "cellImage_0_29": {
            "brand": "Versace",
            "name": "Bright Crystal",
            "description": "Top"
        },
        "cellImage_0_30": {
            "brand": "Versace",
            "name": "Man eau Fraiche",
            "description": "Deluxe"
        },
        "cellImage_0_31": {
            "brand": "Versace",
            "name": "Pour Homme",
            "description": ""
        },
        "cellImage_0_32": {
            "brand": "Marc Antoine Barrois",
            "name": "Ganymede",
            "description": "Top, Deluxe"
        },
        "cellImage_0_33": {
            "brand": "Escentric Molecules",
            "name": "Escentric Molecule 020",
            "description": "Standard, Deluxe, Deluxe New"
        },
        "cellImage_0_34": {
            "brand": "Louis Vuitton",
            "name": "Cosmic Cloude",
            "description": "Top"
        },
        "cellImage_0_35": {
            "brand": "Zarkoperfume",
            "name": "Pink Molecule 090.09",
            "description": "Deluxe"
        },
        "cellImage_0_36": {
            "brand": "Zarkopurfume",
            "name": "MOLéCULE No.8",
            "description": "Standard"
        },
        "cellImage_0_37": {
            "brand": "Parfums de Marley",
            "name": "Perseus ",
            "description": "Top"
        },
        "cellImage_0_38": {
            "brand": "Clive Christian",
            "name": "Rock Rose",
            "description": "Deluxe"
        },
        "cellImage_0_39": {
            "brand": "Clive Christian",
            "name": "Jump up and Kiss me Hedonistic",
            "description": "Deluxe"
        },
        "cellImage_0_40": {
            "brand": "Clive Christian",
            "name": "Matsukita",
            "description": ""
        },
        "cellImage_0_41": {
            "brand": "Montale",
            "name": "Arabians Tonka",
            "description": "Top"
        },
        "cellImage_0_42": {
            "brand": "Orto Parisi",
            "name": "Megamare",
            "description": "Top, Deluxe"
        },
        "cellImage_0_43": {
            "brand": "Orto Parisi",
            "name": "Bergamask",
            "description": "Standard"   
        },
        "cellImage_0_44": {
            "brand": "Ormande Jayne",
            "name": "Montabaco",
            "description": "Deluxe"
        },
        "cellImage_0_45": {
            "brand": "Ormande Jayne",
            "name": "Montabaco Intensivo",
            "description": "Deluxe"
        },
        "cellImage_0_46": {
            "brand": "by Kilian",
            "name": "Sunkissed Goddes",
            "description": "Top"
        },
        "cellImage_0_47": {
            "brand": "by Kilian",
            "name": "Good Girl Gone Bad",
            "description": "Top, Deluxe"
        },
        "cellImage_0_48": {
            "brand": "Maison Crivelli",
            "name": "Hibiscus Mahajad",
            "description": "Deluxe"
        },
        "cellImage_0_49": {
            "brand": "Vilhelm Parfumerie",
            "name": "Mango Skin",
            "description": "Deluxe"
        },
        "cellImage_0_50": {
            "brand": "Hugo Boss",
            "name": "Hugo Boss Bottled Elixir",
            "description": "Top"
        },
        "cellImage_0_51": {
            "brand": "HFC-Haute Fragrance Company",
            "name": "Devil's Intrigue",
            "description": "Top, Deluxe"
        },
        "cellImage_0_52": {
            "brand": "Avon",
            "name": "AVON TODAY",
            "description": "Standard"
        },
        "cellImage_0_53": {
            "brand": "DKNY",
            "name": "Be Delicious",
            "description": "Standard"
        },
        "cellImage_0_54": {
            "brand": "Gucci",
            "name": "Gucci Flora",
            "description": "Standard"
        },
        "cellImage_0_55": {
            "brand": "Elizabeth Arden",
            "name": "GREEN TEA",
            "description": "Standard"
        },
        "cellImage_0_56": {
            "brand": "Rassasi",
            "name": "RUMZ AL RASASI 9325 POUR LUI",
            "description": "Deluxe"
        },
        "cellImage_0_57": {
            "brand": "Hermes",
            "name": "Terre d'Hermes",
            "description": "Standard"
        },
        "cellImage_0_58": {
            "brand": "Lattafa",
            "name": "Sheik Shuyukh",
            "description": "Standard"
        },
        "cellImage_0_59": {
            "brand": "Shaik",
            "name": "Shaik Opulent No 77",
            "description": "Standard"
        },
        "cellImage_0_60": {
            "brand": "Kenzo",
            "name": "Kenzo l'eau Par",
            "description": "Standard"
        },
        "cellImage_0_61": {
            "brand": "Sospiro",
            "name": "Erba Pura",
            "description": "Top"
        },
        "cellImage_0_62": {
            "brand": "Davidoff",
            "name": " Cool Water",
            "description": "Standard"
        },
        "cellImage_0_63": {
            "brand": "Lanvin",
            "name": "Eclat",
            "description": "Standard"
        },
        "cellImage_0_64": {
            "brand": "Armani",
            "name": "Stronger With You ",
            "description": "Top"
        },
        "cellImage_0_65": {
            "brand": "Armani",
            "name": "Stronger With You Intensely",
            "description": "Top"
        },
        "cellImage_0_66": {
            "brand": "Armani",
            "name": "Stronger With You Absolutely",
            "description": "Top"
        },
        "cellImage_0_67": {
            "brand": "Armani",
            "name": "My Way ",
            "description": "Deluxe"
        },
        "cellImage_0_68": {
            "brand": "Armani",
            "name": "Acqua di Gio",
            "description": "Standard"
        },
        "cellImage_0_69": {
            "brand": "Le Labo",
            "name": "Santal 33",
            "description": "Deluxe"
        },
        "cellImage_0_70": {
            "brand": "Carolina Herrera",
            "name": "212 Men",
            "description": "Standard"
        },
        "cellImage_0_71": {
            "brand": "Antonio Banderas",
            "name": "Blue Seduction",
            "description": "Standard"
        },
        "cellImage_0_72": {
            "brand": "Jean Paul Gauiltier ",
            "name": "Le Male Le Parfum",
            "description": "Standard, Top"
        },
        "cellImage_0_73": {
            "brand": "Paco Rabbane",
            "name": "Invictus",
            "description": "Standard"
        },
        "cellImage_0_74": {
            "brand": "Memo",
            "name": "Italian Leather",
            "description": "Top"
        },
        "cellImage_0_75": {
            "brand": "Arabian Oud",
            "name": "Hayati",
            "description": "Standard"
        },
        "cellImage_0_76": {
            "brand": "Calvin Klein",
            "name": "Euphoria Men",
            "description": "Top"
        },
        "cellImage_0_77": {
            "brand": "Calvin Klein",
            "name": "CK One ",
            "description": "Top"
        },
        "cellImage_0_78": {
            "brand": "Montblanc",
            "name": "Explorer",
            "description": "Top"
        },
        "cellImage_0_79": {
            "brand": "Roja Dove",
            "name": "Roja Elysium",
            "description": "Standard"
        },
        "cellImage_0_80": {
            "brand": "Mancera",
            "name": "Cedrat Boise",
            "description": "Top"
        },
        "cellImage_0_81": {
            "brand": "Bvlgari",
            "name": "Tygar",
            "description": "Standard, Top, Deluxe"
        },
        "cellImage_0_82": {
            "brand": "Chanel",
            "name": "Coco Chanel",
            "description": "Top"
        },
        "cellImage_0_83": {
            "brand": "Louis Vuitton",
            "name": "Matiere Noire ",
            "description": "Deluxe"
        },
        "cellImage_0_84": {
            "brand": "Clive Christian",
            "name": "Blonde Amber",
            "description": "Deluxe"
        },
        "cellImage_0_85": {
            "brand": "Gucci",
            "name": "Gardenia Orchid",
            "description": "Top"
        },
        "cellImage_0_86": {
            "brand": "Versace",
            "name": "Versace Fraiche",
            "description": "Deluxe"
        },
        "cellImage_0_87": {
            "brand": "Ex Nihilo",
            "name": "Outcast Blue Extrait",
            "description": "Top"
        },
        "cellImage_0_88": {
            "brand": "Ex Nihilo",
            "name": "Blue Talisman",
            "description": "Top"
        },
        "cellImage_0_89": {
            "brand": "Frederic Malle",
            "name": "Frederic Malle Promice",
            "description": "Top"
        },
        "cellImage_0_90": {
            "brand": "Gucci",
            "name": "Gorgeous Gardenia",
            "description": "Top"
        },
        "cellImage_0_91": {
            "brand": "Givenchy",
            "name": "Givenchy Gentleman",
            "description": "Top"
        },
        "cellImage_0_92": {
            "brand": "Louis Vuitton",
            "name": "Lovers",
            "description": "Top"
        },
        "cellImage_0_93": {
            "brand": "Yves Saint Laurent",
            "name": "Libre",
            "description": "Top"
        },
        "cellImage_0_94": {
            "brand": "Louis Vuitton",
            "name": "Myriad",
            "description": "Top"
        },
        "cellImage_0_95": {
            "brand": "Louis Vuitton",
            "name": "Pacific Chill",
            "description": "Top"
        },
        "cellImage_0_96": {
            "brand": "Maison Crivelli",
            "name": "Oud Stallion",
            "description": "Deluxe"
        },
        "cellImage_0_97": {
            "brand": "MFK",
            "name": "Oud Satin Mood",
            "description": "Top"
        },
        "cellImage_0_98": {
            "brand": "Nishane",
            "name": "Hacivat Nishne",
            "description": "Top, Deluxe"
        },
        "cellImage_0_99": {
            "brand": "Penhaligons",
            "name": "Lord George",
            "description": "Top"
        },
        "cellImage_0_100": {
            "brand": "Parfums de Marley",
            "name": "Layton",
            "description": "Top"
        },
        "cellImage_0_101": {
            "brand": "Parfums de Marley",
            "name": "Althair",
            "description": "Top"
        },
        "cellImage_0_102": {
            "brand": "Louis Vuitton",
            "name": "City of Stars",
            "description": "Top"
        },
        "cellImage_0_103": {
            "brand": "Louis Vuitton",
            "name": "Elves",
            "description": "Deluxe" 
        },
        "cellImage_0_104": {
            "brand": "Yves Saint Laurent",
            "name": "Black Opium",
            "description": "Top"
        },
        "cellImage_0_105": {
            "brand": "Amouage",
            "name": "Guidance",
            "description": "Standard, Top"
        },
        "cellImage_0_106": {
            "brand": "Chanel",
            "name": "Chanel eau Fraiche",
            "description": "Top"
        },
        "cellImage_0_107": {
            "brand": "Tom Ford",
            "name": "Oud Wood",
            "description": "Top"
        },
        "cellImage_0_108": {
            "brand": "Tom Ford",
            "name": "Tuscan Leather",
            "description": "Standard, Top"
        },
        "cellImage_0_109": {
            "brand": "Louis Vuitton",
            "name": "Imagination",
            "description": "Top, Deluxe"
        },
        "cellImage_0_110": {
            "brand": "Dior",
            "name": "Sauvage",
            "description": "Top, Deluxe"
        },
        "cellImage_0_111": {
            "brand": "Creed", 
            "name": "Aventus",
            "description": "Standard, Top, Deluxe"
        },
        "cellImage_0_112": {
            "brand": "Victoria's Secret",
            "name": "Bombshell",
            "description": "Standard, Deluxe"
        },
        "cellImage_0_113": {
            "brand": "Dolce&Gabbana",
            "name": "Light Blue Woman",
            "description": "Top"
        },
        "cellImage_0_114": {
            "brand": "Escentric Molecules",
            "name": "Molecule 04",
            "description": "Top"
        },
        "cellImage_0_115": {
            "brand": "by Kilian",
            "name": "Angel's Share",
            "description": "Standard, Top"
        },
        "cellImage_0_116": {
            "brand": "Maison Crivelli",
            "name": "Oud Maracuja",
            "description": "Standard"
        },
        "cellImage_0_117": {
            "brand": "Britney Spears",
            "name": "Midnight Fantasy",
            "description": "Standard"
        },
        "cellImage_0_118": {
            "brand": "Jean Paul Gauiltier ",
            "name": "Le Male Elixir",
            "description": "Top"
        },
        "cellImage_0_119": {
            "brand": "Essential Parfums",
            "name": "Bois Imperial",
            "description": "Deluxe"
        },
        "cellImage_0_120": {
            "brand": "MFK",
            "name": " Baccarat Rouge 540",
            "description": "Deluxe"
        },
        "cellImage_0_121": {
            "brand": "Louis Vuitton",
            "name": "Symphony",
            "description": "Top"
        },
        "cellImage_0_122": {
            "brand": "Louis Vuitton",
            "name": "California Dream",
            "description": "Top, Deluxe"
        },
        "cellImage_0_123": {
            "brand": "Maison Crivelli",
            "name": "Oud Maracuja",
            "description": "Top"
        },
        "cellImage_0_124": {
            "brand": "Louis Vuitton",
            "name": " Stellar Times",
            "description": "Top, Deluxe"
        },
        "cellImage_0_125": {
            "brand": "Parfums de Marly",
            "name": "Delina",
            "description": "Top, Deluxe"
        },
        "cellImage_0_126": {
            "brand": "Byredo",
            "name": " Bal d’Afrique",
            "description": "Top"
        },
        "cellImage_0_127": {
            "brand": "Louis Vuitton",
            "name": "Meteore",
            "description": "Top"
        },
        "cellImage_0_128": {
            "brand": "Byredo",
            "name": "Mojave Ghost",
            "description": "Top"
        },
        "cellImage_0_129": {
            "brand": "Initio",
            "name": "Oud For Greatness",
            "description": "Top"
        },
        "cellImage_0_130": {
            "brand": "Chanel",
            "name": "Chanel Splendide",
            "description": "Top"
        },
        "cellImage_0_131": {
            "brand": "Creed",
            "name": "Millesime Imperial ",
            "description": "Deluxe"
        },
        "cellImage_0_132": {
            "brand": "Yves Saint Laurent",
            "name": "Y Elixir",
            "description": "Top"
        },
        "cellImage_0_133": {
            "brand": "Initio",
            "name": "Old Fashioned",
            "description": "Top"
        },
        "cellImage_0_134": {
            "brand": "Parfums de Marly",
            "name": "Delina Exclusif",
            "description": "Top"
        },
        "cellImage_0_135": {
            "brand": "Parfums de Marly",
            "name": "Delina Exclusif",
            "description": "Top"
        },
        "cellImage_0_136": {
            "brand": "Loui Vuitton",
            "name": "Dancing Blossom",
            "description": "Top"
        },
        "cellImage_0_137": {
            "brand": "Parfum de Marly",
            "name": "Valaya",
            "description": "Top, Deluxe"
        },
        "cellImage_0_138": {
            "brand": "Creed",
            "name": "Centaurus",
            "description": "Top"
        }
      };
  

const catalogData = Object.entries(catalog).map(([key, value]) => {
  return {
    img: `/assets/${key}.png`,
    brand: value.brand,
    name: value.name,
    description: value.description,
  };
});

export default catalogData;
